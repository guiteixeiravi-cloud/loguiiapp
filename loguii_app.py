...
def carregar_estoque():
    ...
    if os.path.exists(caminho_pdf_min): caminho_correto = caminho_pdf_min
    elif os.path.exists(caminho_pdf_mai): caminho_correto = caminho_pdf_mai
    ...
    if caminho_correto and PyPDF2 is not None:
        try:
            ...
            if itens: return sorted(list(set(itens)))
...
def carregar_pendentes():
    if os.path.exists(arquivo_pendentes):
        try:
            with open(arquivo_pendentes, "r", encoding="utf-8") as f: return json.load(f)
        except: pass
    return []
...
with col_imagem:
    if os.path.exists(caminho_caricatura): st.image(caminho_caricatura, width=130)

if "pendentes" not in st.session_state: st.session_state.pendentes = carregar_pendentes()
...
def atualizar_agrupamentos(pendentes):
    """Agrupa pedidos num raio de 7km (Componentes Conectados)"""
    n = len(pendentes)
    if n == 0: return

    # 1. Garante que todos têm coordenadas
    for p in pendentes:
        if not p.get('lat') or not p.get('lon'):
            lat, lon = geocodificar(f"{p['endereco']}, Franca - SP, Brasil")
            if lat and lon:
                p['lat'], p['lon'] = lat, lon
            time.sleep(0.5) # Pausa para a API do mapa não bloquear

    # 2. Cria a Teia de Aranha (Grafo) de quem está a menos de 7km
    ...
    # 3. Une os grupos (Lógica de Pontes)
    visited = set()
    grupo_id = 1
    for i in range(n):
        if i not in visited:
            ...
            # Verifica se alguém desse grupo já foi travado (nomeado) manualmente
            nome_grupo = None
            for c in componente:
            ...
            # Aplica o nome do grupo a todos os conectados
            for c in componente:
...
def carregar_clientes():
    arquivos = [os.path.join(diretorio_atual, "Listagem_Clientes_de_Franca.xlsx"), os.path.join(diretorio_atual, "Listagem_Clientes_de_Franca.xls"), os.path.join(diretorio_atual, "clientes.xlsx"), "Listagem_Clientes_de_Franca.xlsx"]
    for arquivo in arquivos:
        if os.path.exists(arquivo):
            ...
            except Exception as e: return pd.DataFrame(), False, f"Erro: {str(e)}"
        ...
        if st.sidebar.button("➕ Enviar para Pendentes", type="primary"):
            if cliente_selecionado != "Selecione...":
                with st.spinner("Localizando no mapa e agrupando..."):
                    ...
                    st.rerun()
            else: st.sidebar.warning("Selecione um cliente primeiro.")
else: st.sidebar.error("Planilha de clientes não encontrada!")
...
# Ordena a lista primeiro por Grupo, depois por Urgência
...
if st.session_state.pendentes:
    ...
    for i, p in enumerate(st.session_state.pendentes):
        ...
        # Campo de Grupo (Se o usuário editar, ele trava a rota)
        ...
        if p.get('urgente', False): p['horario'] = col_hor.text_input("Horário:", value=p.get('horario', ''), key=f"hor_{p['uid']}", placeholder="14h00", label_visibility="collapsed")
        else: col_hor.write("")
        ...
        if col_del.button("🗑️", key=f"del_{p['uid']}"):
            ...
            # Reagrupa o resto após excluir alguém, pois a teia pode ter quebrado
            atualizar_agrupamentos(st.session_state.pendentes)
            salvar_pendentes(st.session_state.pendentes)
            st.rerun()
    ...
    with col_btn2:
        if st.button("🔄 Forçar Re-agrupamento (Limpa Nomes Manuais)"):
            for p in st.session_state.pendentes: p['fixo'] = False
            atualizar_agrupamentos(st.session_state.pendentes)
            salvar_pendentes(st.session_state.pendentes)
            st.rerun()
else:
...
# --- ROTINAS DE IMPRESSÃO E MAPA (IDÊNTICAS AO CÓDIGO ANTERIOR) ---
def gerar_pdf(ordem_entregas, nome_motorista):
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=A4)
    largura, altura = A4
    if os.path.exists(caminho_logo): c.drawImage(caminho_logo, 40, altura - 80, width=120, height=45, preserveAspectRatio=True, mask='auto')
...
if st.button("🗺️ Gerar Rota Otimizada", type="primary"):
    if not pedidos_selecionados: st.warning("Selecione pelo menos um endereço na fila para gerar a rota.")
    else:
        with st.spinner("Calculando rotas otimizadas com inteligência de urgência..."):
            ...
            if not (lat_o and lon_o): st.error("Erro ao localizar endereço base.")
            else:
                ...
                if enderecos_validos:
                    ...
                    st.divider()
                    st.download_button(label="📄 Baixar PDF com Checklist da Rota", data=gerar_pdf(ordem_entregas, nome_motorista), file_name=f"LoGuii_Rota_{datetime.now().strftime('%d%m%Y_%H%M')}.pdf", mime="application/pdf", type="primary")
                    ...
                    if os.path.exists(arquivo_historico):
                        try:
                            with open(arquivo_historico, "r", encoding="utf-8") as f: historico = json.load(f)
                        except: pass
                    historico = [r for r in historico if (time.time() - r.get("timestamp", 0)) <= 172800]
                    historico.insert(0, {"timestamp": time.time(), "data_formatada": datetime.now().strftime("%d/%m/%Y %H:%M"), "motorista": nome_motorista or "Não informado", "entregas": ordem_entregas, "link_maps": link_maps})
...
if os.path.exists(arquivo_historico):
    try:
        with open(arquivo_historico, "r", encoding="utf-8") as f: hist = json.load(f)
    except: pass
            ...
            with col_b2:
                if st.button("✏️ Editar (Devolver p/ Fila)", key=f"edit_hist_{i}"):
                    for p_hist in rota['entregas']:
                        p_hist['uid'], p_hist['selecionado'], p_hist['fixo'] = f"hist_{time.time()}_{p_hist['cliente']}", True, False
                        st.session_state.pendentes.append(p_hist)
                    atualizar_agrupamentos(st.session_state.pendentes)
                    salvar_pendentes(st.session_state.pendentes)
                    hist.pop(i)
                    with open(arquivo_historico, "w", encoding="utf-8") as f: json.dump(hist, f, ensure_ascii=False, indent=4)
                    st.rerun()
            with col_b3:
                if st.button("🗑️ Excluir Rota", key=f"del_hist_{i}", type="secondary"):
                    ...
                    with open(arquivo_historico, "w", encoding="utf-8") as f: json.dump(hist, f, ensure_ascii=False, indent=4)
                    st.rerun()
else: st.info("Nenhuma rota salva ainda.")
